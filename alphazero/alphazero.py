...
# TODO make torch?
class StateStack():
    """Stores a numpy state stack containing history
    """

    def __init__(
            self, rows, cols, length_history=4, in_a_row=4, state=None,
        ...
        stack_size = (1 + length_history) * 2 + 1
        if state is None:
            self.state = np.zeros((cols, rows, stack_size), dtype=np.uint8)
            self.first_empty_row = np.zeros((cols,), dtype=np.uint8)
            assert len(self.first_empty_row) == 7
            # self.finished = False
        else:
            self.state = state
            assert first_empty_row is not None
            self.first_empty_row = first_empty_row
            # assert finished is not None
            # self.finished = finished

    def state_after_action(self, action, update=False):
        """Update current state, drop last in history"""

        print("First empty row for state", self.first_empty_row)

        stack_size = self.length_history + 1

        # Establish who is next to play, determines next stack
        if self.state[0, 0, -1] == 0:
            counter = 0
        else:
            counter = stack_size

        # play into last empty row in current col
        if update:
            current_game_state = self.state[:, :, counter]
        else:
            current_game_state = self.state[:, :, counter].copy()

        # play_into_col = new_stack[action, :]
        next_row = self.first_empty_row[action]
        assert next_row < self.rows, "Attempting to take invalid action"
        assert current_game_state[action, next_row] == 0
        current_game_state[action, self.first_empty_row[action]] = 1
        self.first_empty_row[action] += 1

        # print("Stacking", current_game_state.shape, self.state[:, :, counter:counter+self.length_history].shape)

        current_player_new_stack = np.concatenate(
            (np.expand_dims(current_game_state, axis=-1),
            # TODO -1 ?
            self.state[:, :, counter:counter + self.length_history]),
            axis=-1,
        )
        ...
        if update:
            # TODO can it be in place?
            self.state[:, :, counter:counter+stack_size] =\
                current_player_new_stack
            # Change next-to-play marker
            if counter == 0:
                self.state[:, :, -1] = 1
            else:
                self.state[:, :, -1] = 0
            ...
            print("RETURNING NEXT STATE")
            if counter == 0:
                new_stack = np.concatenate(
                    (current_player_new_stack,
                    self.state[:, :, stack_size:stack_size+stack_size],
                    np.expand_dims(self.state[:, :, -1] + 1, axis=-1)),
                    axis=-1
                )
            else:
                new_stack = np.concatenate(
                    (self.state[:, :, 0:stack_size],
                    current_player_new_stack,
                    np.expand_dims(self.state[:, :, -1] - 1, axis=-1)),
                    axis=-1
                )
            print(new_stack, is_finished)
            return new_stack, is_finished

    # TODO - finish this function
    def is_finished(self):
        """Returns true on win, loss, draw"""

        to_play = self.state[0][0]  # 0 or 1
        # add top and bottom 

        if np.all(self.first_empty_row >= self.rows):
            return True

        # Determine if any 4-in-rows
        for player in (0, self.length_history + 1):
            # most recent move - C*R grid
            game_state = self.state[:, :, player]

            # Vertical
            for col in range(game_state.shape[0]):
                in_a_row = 0
                for a in game_state[col, :]:
                    in_a_row = in_a_row + 1 if a == 1 else 0
                    if in_a_row >= 4:
                        # self.finished = True
                        return True
            # vertical
            for row in range(game_state.shape[1]):
                in_a_row = 0
                for a in game_state[:, row]:
                    in_a_row = in_a_row + 1 if a == 1 else 0
                    if in_a_row >= 4:
                        # self.finished = True
                        return True
    ...
    @classmethod
    def from_array(cls, arr, in_a_row=4):
        """
        shape (col, row, (history+1) * 2 + 1)
        arr.shape = (cols, rows, full_depth)
        """
        cols = arr.shape[0]
        assert cols == 7  # temp for testing
        rows = arr.shape[1]
        assert arr.ndim == 3
        state = arr
        assert (arr.shape[-1] - 1) % 2 == 0
        length_history = (arr.shape[-1] - 1) // 2 - 1
        first_empty_row = np.zeros((cols, ), dtype=np.uint8)

        # Make sure no counters in both slots
        for i in range(length_history + 1):
            grid = state[:, :, i] +\
                state[:, :, i + length_history + 1]
        ...
        return cls(
            rows, cols, length_history, state=state, in_a_row=in_a_row,
            first_empty_row=first_empty_row, # finished=False
        )
    
    def __repr__(self):
        ret = "Game stack <StateStack>"
        for i in reversed(range(self.length_history + 1)):
            ret += f"\nMove -{i}\n"
            grid = self.state[:, :, i] +\
                2 * self.state[:, :, i + self.length_history + 1]
            ret += f"{grid}"
        return ret
